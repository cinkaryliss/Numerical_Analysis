...
- (void)drawRect:(NSRect)dRect
{
    ...
    double  yMin = appDelegate.yMin,
            yMax = appDelegate.yMax,
            tMin = appDelegate.tMin,
            tMax = appDelegate.tMax;
    
    if (!autoRange) {
        dRegion.origin.x = tMin;
        dRegion.origin.y = yMin;
        if (tMin != tMax)
            dRegion.size.width = fabs(tMax - tMin);
        if (yMin != yMax)
            dRegion.size.height = fabs(yMax - yMin);
    }
	...
	if (gData.n_data > 0) {
		...
		xd = (double *)[[gData yData] bytes];
		
		[[NSColor colorWithCalibratedRed:0.0 green:0.5 blue:0.0 alpha:1.0] set];
